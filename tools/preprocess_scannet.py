...
def process_scene(scene_dir_src, scene_dir_dst):
    ...
    frames = sorted([f for f in os.listdir(os.path.join(scene_dir_src, 'sensor_data'))
                        if f.endswith('.color.jpg')], key=lambda x: int(x.split('.')[0][-6:]))

    for frame in tqdm.tqdm(frames):
        frame_id = int(frame.split('.')[0][-6:])
        fname_color_src = os.path.join(scene_dir_src, 'sensor_data', frame)
        fname_depth_src = os.path.join(scene_dir_src, 'sensor_data', 'frame-{0:06d}.depth.png'.format(frame_id))
        ...
        P = np.loadtxt(os.path.join(scene_dir_src, 'sensor_data', 'frame-{0:06d}.pose.txt'.format(frame_id)))
